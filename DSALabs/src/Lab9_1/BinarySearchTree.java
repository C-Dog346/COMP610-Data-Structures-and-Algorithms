...
public class BinarySearchTree<E> extends AbstractSet<E>
    ...
    public Iterator<E> iterator() {
        ...
        String order = "levelOrder";
        return new BinaryTreeIterator(rootNode, order);
    }
...
//    To perform a correct rotation of a node z - look at its 
//    child y on the path to the newly added node
//    if z = +2 and y > 0, outer left too heavy, right rotation
//    if z += 2 and y < 0, inner left too heavy, left rotation, right rotation
//    if z = -2 and y < 0, outer right too heavy, left rotation
//    if z = -2 and y > 0, inner right too heavy, right rotation, left rotation
    
    public void leftRotateRoot() {
        BinaryTreeNode oldRoot = rootNode;
        BinaryTreeNode newRootNode = rootNode.rightChild;
        BinaryTreeNode floatingNode = newRootNode.leftChild;
        
        newRootNode.leftChild = null;
        rootNode.rightChild = null;
        newRootNode.leftChild = oldRoot;
        oldRoot.rightChild = floatingNode;
        rootNode = newRootNode;
        
//        rootNode.rightChild = floatingNode;
//        rootNode.rightChild.leftChild = rootNode;
//        rootNode = newRootNode;
    }

    public void rightRotateRoot() {
        BinaryTreeNode oldRoot = rootNode;
        BinaryTreeNode newRootNode = rootNode.leftChild;
        BinaryTreeNode floatingNode = newRootNode.rightChild;
        
        newRootNode.rightChild = null;
        rootNode.leftChild = null;
        newRootNode.rightChild = oldRoot;
        oldRoot.leftChild = floatingNode;
        rootNode = newRootNode;
    }
    ...
    private class BinaryTreeIterator implements Iterator<E> {
        ...
        public BinaryTreeIterator(BinaryTreeNode rootNode, String order) {   // testing is commented inside of Iterator in BST class
        ...
        private void traverseLevelOrder(BinaryTreeNode node) {
            ...
            BinaryTreeNode frontNode;
            q.add(node);
            while (!q.isEmpty()) {
                frontNode = q.peek();
                if (frontNode.leftChild != null) {
                    q.add(frontNode.leftChild);
                }
                if (frontNode.rightChild != null) {
                    q.add(frontNode.rightChild);
                }
                if (withinView(frontNode.element)) {
                    list.add(frontNode.element);
                }
                q.poll();
            }
